...
function generateUnifiedExecutionSteps(codeText, realTimeMetrics, realExecutionTime) {
    ...
    // Analyze code complexity to determine which steps to include
    ...
    // Count active steps dynamically
    let activeSteps = 1; // Always include parsing
    ...
    // Step 1: Code Parsing & Lexical Analysis (always included)
    const lineCount = (codeText.match(/\n/g) || []).length + 1;
    const memoryForParsing = (lineCount * 0.1).toFixed(2);
    steps.push({
        icon: '📝',
    ...
    // Step 2: Module Import & Dependency Resolution (for larger codebases)
    ...
    // Step 3: Variable Scope & Memory Allocation (for larger codebases)
    ...
    // Step 4: Function Analysis & Call Stack Setup
    if (functionPatterns.total > 0) {
        const functionMemory = (functionPatterns.total * 2.5).toFixed(2);
        steps.push({
            icon: '🔧',
    ...
    // Step 5: Loop Processing & Iteration Optimization
    if (loopPatterns.total > 0) {
    ...
    // Step 6: Async Operations & Promise Resolution
    if (asyncPatterns.total > 0) {
    ...
    // Step 7: DOM Manipulation & Event Binding
    if (domPatterns.total > 0) {
    ...
    // Step 8: Error Handling & Exception Management (for complex code)
    if (errorPatterns.total > 0 || isComplexCodebase) {
        const errorStatus = errorPatterns.unhandled > 0 ? 'warning' : 'complete';
        const errorMemory = (errorPatterns.total * 1.2).toFixed(2);
        steps.push({
            icon: '🛡️',
            name: 'Error Handling & Exception Management',
            time: `${stepTime.toFixed(2)}ms`,
            status: errorStatus,
            details: `${errorPatterns.tryCatch} try/catch blocks, ${errorPatterns.throws} throw statements`,
            metrics: `Error memory: ~${errorMemory}KB | Coverage: ${errorPatterns.coverage}%`
        });
    }

    // Step 9: Security Analysis & Vulnerability Check (for complex code)
    if (securityPatterns.total > 0 || isComplexCodebase) {
        const securityStatus = securityPatterns.high > 0 ? 'error' : (securityPatterns.medium > 0 ? 'warning' : 'complete');
        steps.push({
            icon: '🔒',
            name: 'Security Analysis & Vulnerability Check',
            time: `${stepTime.toFixed(2)}ms`,
            status: securityStatus,
            details: `${securityPatterns.severity.high} high-risk, ${securityPatterns.severity.medium} medium-risk issues found`,
            metrics: `Security score: ${securityPatterns.score}/100 | Patterns: ${securityPatterns.total}`
        });
    }

    // Step 10: Performance Profiling & Optimization (for complex code)
    if (performancePatterns.total > 0 || isComplexCodebase) {
        const perfStatus = performancePatterns.critical > 0 ? 'error' : (performancePatterns.warnings > 0 ? 'warning' : 'complete');
        steps.push({
            icon: '⚡',
            name: 'Performance Profiling & Optimization',
            time: `${stepTime.toFixed(2)}ms`,
            status: perfStatus,
            details: `${performancePatterns.bottlenecks} bottlenecks, ${performancePatterns.optimizations} optimization opportunities`,
            metrics: `Perf score: ${performancePatterns.score}/100 | Critical: ${performancePatterns.critical}`
        });
    }

    // Step 11: Garbage Collection & Memory Cleanup (for larger codebases)
    if (realTimeMetrics.gcCollections > 0 || isLargeCodebase) {
        const finalMemory = (realTimeMetrics.peakMemory / (1024 * 1024)).toFixed(2);
        const gcStatus = parseFloat(finalMemory) > 50 ? 'warning' : 'complete';
        steps.push({
            icon: '🗑️',
            name: 'Garbage Collection & Memory Cleanup',
            time: `${stepTime.toFixed(2)}ms`,
            status: gcStatus,
            details: `${realTimeMetrics.gcCollections} GC cycles, memory optimized`,
            metrics: `Final memory: ${finalMemory}MB | Freed: ~${(realTimeMetrics.gcCollections * 2.5).toFixed(2)}KB`
        });
    }

    // Time verification and adjustment
    const calculatedTotal = steps.reduce((sum, step) => {
        const timeValue = parseFloat(step.time.replace('ms', ''));
        return sum + timeValue;
    }, 0);
...
function detectModulePatterns(codeText) {
    const imports = (codeText.match(/import\s+.*?from\s+['"`]/g) || []).length;
    const exports = (codeText.match(/export\s+(?:default\s+)?/g) || []).length;
    const dynamic = (codeText.match(/import\s*\(/g) || []).length;
    const requires = (codeText.match(/require\s*\(/g) || []).length;
    ...
    return {
        ...
        dynamic,
        requires,
        total: imports + exports + dynamic + requires
    };
}

// Variable scope analysis
function detectVariablePatterns(codeText) {
    const locals = (codeText.match(/(?:let|const|var)\s+\w+/g) || []).length;
    const globals = (codeText.match(/(?:^|\n)\s*\w+\s*=/g) || []).length;
    const constants = (codeText.match(/const\s+\w+/g) || []).length;
    const closures = (codeText.match(/function.*?\{[\s\S]*?function/g) || []).length;
    ...
    return {
        ...
        constants,
        closures,
        total: locals + globals + constants
    };
}

// Code complexity analysis
function analyzeCodeComplexity(codeText) {
    const lines = (codeText.match(/\n/g) || []).length + 1;
    const functions = (codeText.match(/function\s+\w+|=>\s*\{|=>\s*\w+/g) || []).length;
    const conditions = (codeText.match(/if\s*\(|switch\s*\(|case\s+/g) || []).length;
    const loops = (codeText.match(/for\s*\(|while\s*\(|do\s*\{/g) || []).length;
    const tokens = codeText.split(/\s+/).length;

    // Cyclomatic complexity approximation
    const cyclomatic = 1 + conditions + loops;
...
function detectErrorPatterns(codeText) {
    const tryCatch = (codeText.match(/try\s*\{/g) || []).length;
    const throws = (codeText.match(/throw\s+/g) || []).length;
    const catches = (codeText.match(/catch\s*\(/g) || []).length;
    const finally_ = (codeText.match(/finally\s*\{/g) || []).length;

    const total = tryCatch + throws + catches + finally_;
    const coverage = tryCatch > 0 ? Math.min(100, (catches / tryCatch) * 100) : 0;
    const unhandled = throws - catches;
...
function analyzePerformanceIssues(codeText) {
